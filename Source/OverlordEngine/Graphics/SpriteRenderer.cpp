...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	//Sort Sprites
	//SORT BY TEXTURE
	sort(m_Sprites.begin(), m_Sprites.end(), [this](const SpriteVertex& v0, const SpriteVertex& v1)
	{
		return v0.TextureId < v1.TextureId;
	});
	...
	memcpy(mappedResource.pData, &m_Sprites[0], sizeof(SpriteVertex) * m_Sprites.size());
...
void SpriteRenderer::DrawImmediate(const GameContext& gameContext, ID3D11ShaderResourceView* pSrv, XMFLOAT2 position, XMFLOAT4 color, XMFLOAT2 pivot, XMFLOAT2 scale, float rotation)
{
	...
	//Set Texture Size
	m_pTextureSizeV->SetFloatVector(reinterpret_cast<float*>(&XMFLOAT2(texDesc.Width, texDesc.Height)));
