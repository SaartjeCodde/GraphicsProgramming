...
VS_OUTPUT VS(VS_INPUT input){
	...
	{
		if (input.blendIndices[index] > -1)
		{
			transformedPosition += (input.blendWeights[index] * mul(originalPosition, gBones[input.blendIndices[index]]));
							
			transformedNormal += input.blendWeights[index] * normalize(mul(input.normal, (float3x3)gWorld));
		}
	}
