...
{
	// 1. Delete this transformation code and just return the VS_DATA parameter 
...
{
	// 1. Create a GS_DATA object
	GS_DATA gOut;
	// 2. Transforms position using the WVP Matrix and assign it to (GS_DATA object).Position 
	gOut.Position = mul(float4(pos, 1.f), m_MatrixWorldViewProj);
	// 3. Transform the normal using the World Matrix and assign it to (GS_DATA object).Normal 
	gOut.Normal = mul(normal, (float3x3)m_MatrixWorld);
	// 4. Assign texCoord to (GS_DATA object).TexCoord
	gOut.TexCoord = texCoord;
	// 5. Append (GS_DATA object) to the TriangleStream parameter
...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
	...
	// 1. Calculate The basePoint
	basePoint = (vertices[0].Position + vertices[1].Position + vertices[2].Position)/3;
	// 2. Calculate The normal of the basePoint
	float3 basePointNormal = (vertices[0].Normal + vertices[1].Normal + vertices[2].Normal)/3;
	// 3. Calculate The Spike's Top Position
	top = basePoint + (8 * basePointNormal);
	// 4. Calculate The Left And Right Positions
	float3 spikeDirection = (vertices[2].Position - vertices[0].Position) * 0.1;
	left = basePoint - spikeDirection;
	right = basePoint + spikeDirection; 
	// 5. Calculate The Normal of the spike
